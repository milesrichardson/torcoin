...
[THIS SECTION NEEDS CITATIONS. Namely, citations identifying papers that fail to
conform to the constraints we name.]
...
identify the source or destination of any packet it transfers. An acceptable
implementation of TorCoin will at least retain the anonymity of the current Tor
protocol, but an excellent one will improve upon it.
...
will not require self-reporting or centralized servers, alike EigenSpeed
\cite{jansen2013lira} or Opportunistic Bandwidth Monitoring.
 \cite{snader2008tune} It should not be possible for attackers or groups of
attackers to misreport bandwidth measurements, and the entire network should
agree on all measurements.
...
same bandwidth transfer. Fortunately, the BitCoin protocol provides a
distributed ledger for tracking transactions, which TorCoin will rely upon.

\subsubsection{Deployable with Minimal Code Changes} To simplify deployability,
TorCoin should operate mostly as a standalone system from Tor. It should
require minimal changes to the Tor core codebase, and should not significantly
