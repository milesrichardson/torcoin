...
traffic to be identified. An acceptable implementation of TorCoin will at
...
\subsection{Challenges} A naive bandwdth-measurement scheme that uses blinded
cryptographic tokens to signify bandwidth transfer is vulnerable to colluding
groups of clients and relays who can simply sign each other's transfer tokens
without actually transferring any bandwidth. 

This is countered by the TorPath scheme which restricts clients' ability to
choose their own path. Assignment Servers bundle large groups of clients and
relays into groups to collectively choose paths. Even if colluding clients and
relays are assigned to a path (low probability), a rate-limiting system is
able to limit the number of coins such a path can mint.
